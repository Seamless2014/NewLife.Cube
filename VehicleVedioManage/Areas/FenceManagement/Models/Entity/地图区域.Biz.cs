...
namespace VehicleVedioManage.FenceManagement.Entity
{
    public partial class MapArea : Entity<MapArea>
    {
        ...
        /// <returns></returns>
        public static IList<MapArea> FindByDelete(bool isDelete)
        {
            // 实体缓存
            if (Meta.Session.Count < 1000) return Meta.Cache.FindAll(e => e.Deleted==isDelete);

            // 单对象缓存
            //return Meta.SingleCache.GetItemWithSlaveKey(name) as MapArea;

            return FindAll(_.Deleted == isDelete);
        }
        #endregion
