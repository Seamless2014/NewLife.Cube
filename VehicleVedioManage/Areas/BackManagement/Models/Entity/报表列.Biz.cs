...
{
    public partial class ReportColumn : Entity<ReportColumn>
    {
        #region 对象操作
        static ReportColumn()
        {
        ...
        //    if (XTrace.Debug) XTrace.WriteLine("开始初始化ReportColumn[报表列]数据……");
        ...
        //    if (XTrace.Debug) XTrace.WriteLine("完成初始化ReportColumn[报表列]数据！");
        //}
        ...
        #region 扩展查询
        /// <summary>根据报表列编码查找</summary>
        /// <param name="reportColumnId">报表列编码</param>
        /// <returns>实体对象</returns>
        public static ReportColumn FindByReportColumnId(Int32 reportColumnId)
        {
            if (reportColumnId <= 0) return null;

            // 实体缓存
            if (Meta.Session.Count < 1000) return Meta.Cache.Find(e => e.ReportColumnId == reportColumnId);

            // 单对象缓存
            return Meta.SingleCache[reportColumnId];
