...
(function ($) {
    var settings = {}, roots = {}, caches = {},
        ...
        _consts = {
            ...
            icon_map: {
                "province": "icon-province",
                "city": "icon-city",
                "county": "icon-county",
                "area": "icon-area",
                "area-child": "icon-area-child",
                "enterprise": "icon-enterprise",
                "car": "icon-car",
                "carwithcamera": "icona-4G",
                "dep": "icon-dep",
                "agent": "icon-agent"
            },
            icon_color_map: {
        ...
        data = {
            ...
            nodeTextIcon: function (setting, node) {
                var nameKey = setting.data.key.textIcon;
                return node[nameKey];
            },
            nodeIconColor: function (setting, node) {
                var nameKey = setting.data.key.iconColor;
                return node[nameKey];
            },
        ...
        view = {
            addNodes: function (setting, parentNode, index, newNodes, isSilent) {
                ...
                if (parentNode) {
                    ...
                    if (!parentNode.open) {
                        if (!data.nodeTextIcon(setting, parentNode)) {
            ...
            expandCollapseNode: function (setting, node, expandFlag, animateFlag, callback) {
                ...
                if (isParent) {
                    ...
                    if (node.open) {
                        if (!data.nodeTextIcon(setting, node)) {
            ...
            makeDOMNodeIcon: function (html, setting, node) {
                var nameStr = data.nodeName(setting, node),
                    isTextIcon = data.nodeTextIcon(setting, node),
                    iconColor = data.nodeIconColor(setting, node),
                    textIcon = isTextIcon ? "iconfont " + consts.icon_map[isTextIcon] + " " + consts.icon_color_map[iconColor] : view.makeNodeIcoClass(setting, node),
                    //textIcon = isTextIcon ? "iconfont " + consts.icon_map[isTextIcon] + "icon-enterprise car-online" isTextIcon: view.makeNodeIcoClass(setting, node),
                    style = isTextIcon ? "" : view.makeNodeIcoStyle(setting, node),
                    name = setting.view.nameIsHTML ? nameStr : nameStr.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
                html.push("<span id='", node.tId, consts.id.ICON,
                    "' title='' treeNode", consts.id.ICON, " class='", textIcon,
                    "' style='", style, "'></span><span id='", node.tId, consts.id.SPAN,
...
(function ($) {
    ...
    var _consts = {
        ...
        _zTreeTools = function (setting, zTreeTools) {
            ...
            zTreeTools.getCheckedNodeIDs = function (checked) {
                checked = (checked !== false);
                var children = data.nodeChildren(setting, data.getRoot(this.setting));
                return data.getTreeCheckedNodeIDs(this.setting, children, checked);
            }

            zTreeTools.getCheckedLeafNodes = function (checked) {
                checked = (checked !== false);
                var children = data.nodeChildren(setting, data.getRoot(this.setting));
                return data.getTreeCheckedLeafNodes(this.setting, children, checked);
            }

            zTreeTools.getCheckedLeafNodeIDs = function (checked) {
                checked = (checked !== false);
                var children = data.nodeChildren(setting, data.getRoot(this.setting));
                return data.getTreeCheckedLeafNodeIDs(this.setting, children, checked);
            }

            zTreeTools.getOpenedNodeIDs = function () {
                var children = data.nodeChildren(setting, data.getRoot(this.setting));
                return data.getTreeOpenedNodeIDs(this.setting, children);
            }
        ...
        _data = {
            ...
            getTreeCheckedNodeIDs: function (setting, nodes, checked, results) {
                if (!nodes) return [];
                var onlyOne = (checked && setting.check.chkStyle == consts.radio.STYLE && setting.check.radioType == consts.radio.TYPE_ALL);
                results = !results ? [] : results;
                for (var i = 0, l = nodes.length; i < l; i++) {
                    ...
                    data.getTreeCheckedNodeIDs(setting, children, checked, results);
                    if (onlyOne && results.length > 0) {
                        break;
                    }
                }
            ...
            getTreeCheckedLeafNodes: function (setting, nodes, checked, results) {
                if (!nodes) return [];
                var onlyOne = (checked && setting.check.chkStyle == consts.radio.STYLE && setting.check.radioType == consts.radio.TYPE_ALL);
                if (onlyOne) return [];
                results = !results ? [] : results;
                for (var i = 0, l = nodes.length; i < l; i++) {
                    ...
                    if (children !== undefined && children !== null && children.length > 0) {
                        data.getTreeCheckedLeafNodes(setting, children, checked, results);
                    }
                    else {
                        var nodeChecked = data.nodeChecked(setting, node);
                        if (node.nocheck !== true && node.chkDisabled !== true && nodeChecked == checked) {
            ...
            getTreeCheckedLeafNodeIDs: function (setting, nodes, checked, results) {
                if (!nodes) return [];
                var onlyOne = (checked && setting.check.chkStyle == consts.radio.STYLE && setting.check.radioType == consts.radio.TYPE_ALL);
                if (onlyOne) return [];
                results = !results ? [] : results;
                for (var i = 0, l = nodes.length; i < l; i++) {
                    ...
                    var children = data.nodeChildren(setting, node);
                    if (children !== undefined && children !== null && children.length > 0) {
                        data.getTreeCheckedLeafNodeIDs(setting, children, checked, results);
                    }
                    else {
                        var nodeChecked = data.nodeChecked(setting, node);
                        if (node.nocheck !== true && node.chkDisabled !== true && nodeChecked == checked) {
            ...
            getTreeOpenedNodeIDs: function (setting, nodes, results) {
                if (!nodes) return [];
                results = !results ? [] : results;
                for (var i = 0, l = nodes.length; i < l; i++) {
                    var node = nodes[i];
                    ...
                    data.getTreeOpenedNodeIDs(setting, children, results);
                }
                return results;
            },
