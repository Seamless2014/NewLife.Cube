...
AlarmGrid.refreshInterval = 5; //报警刷新时间间隔
AlarmGrid.displayTime = null;
AlarmGrid.autoRefresh = false;
AlarmGrid.popNum = 0;
AlarmGrid.showVehicleOnMap = true;//报警时候显示车辆在地图上
AlarmGrid.maxPopupNum = 20;//最多弹窗的报警条数，超过后，前面的要清除避免占用内存

AlarmGrid.alarmMap = {};

AlarmGrid.create = function(alarmInterval, showVehicleOnMap)
{
    ...
    this.alarmDataGrid.datagrid({
				...
				queryParams: { 'queryId': 'selectProcessedAlarms','processedState':'all' },
				idField: 'id',
				striped: true,
				loadMsg:'',
				fitColumns: true,
	...
	$("#btnQueryAlarmData").click(function()
		{
			...
			//var today = Utility.now(); 不能取客户端时间
			//只查询当天的报警
			me.params = {today:"yes",plateNo:plateNo, alarmType:alarmType,alarmSource:alarmSrc,queryId:"selectProcessedAlarms",'processedState':'all'};
	...
	$("#btnResetAlarm").click(function()
		{
			...
			me.params = {queryId:"selectProcessedAlarms",'processedState':'all'};
...
AlarmGrid.onDblClickRow = function(rowIndex, rowData)
{
	...
	var url =globalConfig.webPath  + "/Alarm.mvc/viewAlarm?alarmId="+ alarmId;
...
AlarmGrid.addNewAlarm = function(alarmData)
		{
			...
			{			
				...
				$.each(alarmArray, function (i, alarm) {
                       ...
                       if (alarm.alarmSource == "平台报警")
                          AlarmGrid.alarmMap[alarm.plateNo] = alarm.alarmTypeDescr;
				...
				//如果设置为弹窗
				if(me.showVehicleOnMap && lastAlarm.alarmType != 'GpsOnline' && lastAlarm.alarmType != 'GpsOffline')
				{
				    MyMap.showVehicleAlarmOnMap(lastAlarm.vehicleId,lastAlarm.alarmTypeDescr); //调用main.jsp的jS方法,显示车辆在地图中心
				}
				if(soundEnabled)
			...
			if(alarmData.jt809Notify && alarmData.jt809Notify.length > 0)
			{
				...
				$.each(alarmArray, function (i, alarm) {
					...
					{
						var url = globalConfig.webPath+"/PlatformPostQuery.mvc/postQuery?commandId="+alarm.cmdId;
						InfoWindow.open(url, 630,350,"上级平台查岗");
					}else if(strType == "0x9302") //查岗请求，弹出查岗的窗口
					{
						var url = globalConfig.webPath+"/PlatformPostQuery.mvc/platformText?commandId="+alarm.cmdId;
			...
			if(alarmData.warnMsgTodoReq && alarmData.warnMsgTodoReq.length > 0)
			{
				...
				$.each(alarmArray, function (i, alarm) { //单纯的遍历数组    
					...
					var url = globalConfig.webPath+"/JT809AlarmToDo.mvc/View?msgId="+alarm.id;
...
{
	var url = globalConfig.webPath+"/Alarm.mvc/alarm";
	var me = this;
	var params = {};
	 $.ajax({
            type: "POST",
            ...
            success: function(data){
			   ...
			   {
				   window.location = globalConfig.webPath+"/Home.mvc/loginout";
			   }
            }
        });
}
