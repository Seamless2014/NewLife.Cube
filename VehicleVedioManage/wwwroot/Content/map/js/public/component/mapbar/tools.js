...
LTHand.prototype = {
	btnClick : function(){
		...
		this._handler.maplet.setMode("pan");
	}
}
	...
	btnClick : function() {
		try{
			...
			if (!this._flag) {
				...
				switch(this._type){
				case "polyline":
					this._bindHandler = this._handler.maplet.setMode("drawline", function(dt){
						if (handler._drawPolylineEnd){
							var polyline = handler.createPolyline(dt.points);
							handler.addOverlay(polyline);
							co._prePolyline = polyline;
							handler._drawPolylineEnd(handler.pointToString(dt.points), handler.formatPointsToArray(dt.points), dt.points.length, polyline, co);
						}
					});
					break;
				case "measure":
					this._handler.maplet.setMode("measure");
					break;
				}
			} else {
				if (this._bindHandler != null)handler.removeListener(this._bindHandler);
	...
	btnClick : function() {
		try{
			...
			if (!this._flag) {
				...
				switch(this._type){
				case "rect":
					this._bindHandler = this._handler.maplet.setMode("lookup", function(dt){
						var Xmin = dt.min.lon;
						var Ymin = dt.min.lat;
						var Xmax = dt.max.lon;
						var Ymax = dt.max.lat;
						...
						// alert(handler._drawRectEnd);
						var rect = handler.createRect(dt);
						handler.addOverlay(rect);
						
						if (handler._drawRectEnd) {
							handler._drawRectEnd(handler.pointToString(points, true), null, rect, points, 2);
						}
					});
					
					break;
				default:
					this._handler.maplet.setMode(this._type);
				}
				...
				this._bindHandler = this._handler.maplet.setMode("lookup", function(dt){
					...
					if (window.parent.Srims) {
						window.parent.Srims.command.areaComponents.tool.rect.handler.areaQueryHandler(
							handler.pointToString(points, true), null, rect, points, 2, [
					...
					this._bindHandler = handler.eventBindObject(this._extObj, "drawend", handler._oMap, function(c){
						...
						if (handler._drawCircleEnd){
							handler._drawCircleEnd(
								handler.pointToString(points, true), 
								null, 
								c, 
								points, 
								1
				...
				case "polygon":
					...
					this._bindHandler = this._handler.maplet.setMode("drawline", function(dt){
						if (handler._drawPolygonEnd){
							var polygon = handler.createPolygon(dt.points);
							handler.addOverlay(polygon);
							handler._drawPolygonEnd(handler.pointToString(dt.points, true), null, polygon, dt.points, 3);
						}
					});
					break;
				}
