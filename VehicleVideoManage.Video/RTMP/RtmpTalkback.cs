...
namespace VehicleVideoManage.Video.RTMP
{
    public class RtmpTalkback
    {
        ...
        private VideoFileUtility fileUtil = new VideoFileUtility();
        ...
        public RtmpTalkback(ITracer _tracer)
        {
            this.tracer = _tracer;
        }
        ...
        public void Stop()
        {
            ...
            try
            {
                ...
                {
                    tracer.NewSpan("开始释放AAC解码器:" + SimNo);
        ...
        private bool AutoReconnect()
        {
            if (rtmpHandle == IntPtr.Zero || RTMPWrapper.RTMP264_IsConnected(rtmpHandle) == 0)
            {
                ...
                tracer.NewSpan("语音对讲：" + RtmpLocation + ",重连接" + result);
            }
        ...
        private void pull()
        {
            try
            {
                ...
                tracer.NewSpan("语音对讲：" + RtmpLocation + ",音频编码:" + AudioCoder.GetAudioCoderDescr(audioEncoder) + ",海思格式:" + HISIHead + "," + result);
                int bufsize = 1024;
                    ...
                    while (continuePull)
                    {
                        try
                        {
                            ...
                            while (continuePull && (nRead = RTMPWrapper.RTMP264_ReadPacket(rtmpHandle, ptrBuffer, ref packetType, bufsize)) > 0)
                            {
                                ...
                                if (firstStream)
                                {
                                    tracer.NewSpan("开始语音对讲：" + RtmpLocation + ",音频编码:" + AudioCoder.GetAudioCoderDescr(audioEncoder) + ",海思格式:" + HISIHead);
                                    firstStream = false;
                                }
                                            ...
                                            if (frame[1] == 0)
                                            {
                                                ...
                                                if (adsAACDecoderSpecific.nAudioFortmatType == 10)
                                                {
                                                    ...
                                                    if (adsAACDecoderSpecific.nAudioSampleType == 3)
                                                    {
                                                        ...
                                                        {
                                                            tracer.NewSpan("初始化重采样器失败,错误吗:" + err);
                                                        }
                                                    }
                                                }
                            ...
                            {
                                tracer.NewSpan(RtmpLocation + "关闭对讲拉流，等待再次重连");
                                lock (rtmpHandleLock)
                                {
                                    if (rtmpHandle != IntPtr.Zero)
                                    {
                                ...
                                if (tryTimes++ > 3)
                                {
                                    tracer.NewSpan(RtmpLocation + "重连接超过3次主动关闭");
                                }
                            }
                        }
                        catch (Exception ex2)
            ...
            tracer.NewSpan("语音对讲：" + RtmpLocation + ",音频编码:" + AudioCoder.GetAudioCoderDescr(audioEncoder) + ",海思格式:" + HISIHead + ",退出");
        }
        ...
        private void EncoderAudioData(byte[] pcmData)
        {
            ...
            while (pcmBufferOffset >= AUDIO_ENCODER_BUFFER_SIZE)
            {
                ...
                {
                    tracer.NewSpan(AudioCoder.GetAudioCoderDescr(audioEncoder) + "编码错误");
                }
