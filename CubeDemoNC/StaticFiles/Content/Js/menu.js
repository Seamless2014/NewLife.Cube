...
MyMenu.createCommandToolBar = function(cmdMenu)
	{
		$.each(cmdMenu, function (i, sm) {
			...
			var url = sm.url;
			var icon = sm.icon;
			var linkbutton =  "<a onclick=\"MyMenu.onCommandToolbarClick('"+menuCode +"','"+ menuName+"','"+ icon+"','"+ url+"')\" class='easyui-linkbutton'  href='#' data-options=\"iconCls:'"+sm.icon+"',plain:false\">"+sm.text+"</a>";
			$("#commandBar").append($(linkbutton));
			$("#commandBar").append($("<span style='width:10px;'></span>"));
...
MyMenu.createRightMenu = function(rightMenuData)
	{
		...
		$.each(rightMenuData, function (i, sm) {
			...
			$('#rightMenu').menu('appendItem', {
					...
					iconCls: sm.icon,
					onclick: function(){
						me.onRightMenuClick(menuCode,menuName,icon,url);//菜单点击事件
					}
				});
        });
		return $("#rightMenu");
...
MyMenu.setMapPath = function(mapType) {
		if(mapType == globalConfig.mapType)
			return;
		var mapPath = globalConfig.mapPath;
		//globalConfig.mapPath = mapPath;
	    globalConfig.mapType = mapType;

		var url = globalConfig.webPath+"/User.mvc/setMapType";
    ...
    //右键菜单点击事件
MyMenu.onRightMenuClick = function(menuCode,menuName,icon,url)
	{
		var node = VehicleTree.getSelected();
		var vehicleId = node.id.substring(1);
		var title = menuName + "["+node.text + "]";
		InfoWindow.openCommandWindow(menuCode,vehicleId,title,icon,url,node.text);
		
			HistoryRoutePanel.setPlateNo(node.text);
	}
	//命令工具栏点击事件
MyMenu.onCommandToolbarClick = function(menuCode, menuName,icon,url)
	{
		
		if(menuCode == 'VEHICLE_IN_AREA_TIME')
		{
	         addTab(menuName,url,"icon-track");
			 return;
		}
		//var node = VehicleTree.getSelected();
		...
		InfoWindow.openCommandWindow(menuCode,vehicleId,title,icon,url);
	}
